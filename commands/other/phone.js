...
module.exports = class PhoneCommand extends Command {
	...
	async run(msg, { channelID }) {
		if (channelID !== 'count' && (!msg.channel.topic || !msg.channel.topic.includes('<xskull:phone>'))) {
			return msg.say('You can only start a call in a channel with `<xskull:phone>` in the topic.');
		}
		...
		if (channelID) {
			...
			if (!channel.topic || !channel.topic.includes('<xskull:phone>')) {
